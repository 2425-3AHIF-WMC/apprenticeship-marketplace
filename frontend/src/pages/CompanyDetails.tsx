...
import { Building, Mail, Phone, Globe, Search, ArrowLeft } from 'lucide-react';
...
const CompanyDetails = () => {
  ...
  const backPath = location.state?.backPath || '/companies';
  const [company, setCompany] = useState<CompanyUIPropsAdmin | null>(null);
  const [companyLoading, setCompanyLoading] = useState(!location.state?.company);
  ...
  useEffect(() => {
    const fetchInternships = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:5000/api/company/${id}/internships`);
        ...
        setInternshipsError(err.message || 'Unbekannter Fehler');
      } finally {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {internships.map((internship, index) => (
                    <FadeIn key={internship.id} delay={index * 50}>
                      <InternshipCard internship={internship} backPath={`/companies/${id}`} />
                    </FadeIn>
                  ))}
                </div>
              ) : (
